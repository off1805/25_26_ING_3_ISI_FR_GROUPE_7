// AP breadcrumbs:
// - Navbar: "AP > <active sidebar item>"
// - Page: "<active sidebar item> > <full trail>" from data-ap-page-bc, fallback to sidebar
//
// data-ap-page-bc format (optional):
...
function getSidebarActiveItem() {
  const sidebar = document.querySelector("[data-ap-sidebar]");
...
function renderNavbarBreadcrumb(container, items) {
  ...
  const all = [{ label: "AP" }, ...items.filter((it) => it && it.label)];
  if (all.length === 1) {
    container.innerHTML = `${iconHome()}<span class="font-medium text-layer-foreground">AP</span>`;
    return;
  }
...
function fillBreadcrumbs() {
  ...
  const navbarContainer = document.querySelector('[data-ap-breadcrumb-target="navbar"]');
  const pageContainer = document.querySelector('[data-ap-breadcrumb-target="page"]');

  const rawPage = document.body.getAttribute("data-ap-page-bc");
