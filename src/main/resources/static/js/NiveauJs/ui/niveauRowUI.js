export function createTaskRow(niveau) {
    return `
        ...
        <div class="w-full flex justify-between items-center truncate">
            <div class="flex items-center gap-x-3">
                <div class="size-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                    <span class="text-primary font-bold">${niveau.ordre}</span>
                </div>
                <div class="truncate">
                    <a href="/admin/niveaux/${niveau.id}" class="font-bold text-primary hover:underline">Niveau ${niveau.ordre}</a>
                    <span class="text-muted-foreground-2 block text-xs">${niveau.description || ''}</span>
                </div>
            </div>
            <div class="hs-dropdown relative inline-flex">
                ...
                <div class="hs-dropdown-menu transition-[opacity,margin] duration-200 hs-dropdown-open:opacity-100 opacity-0 hidden min-w-36 bg-layer border border-card-line shadow-lg rounded-xl z-20 p-1">
                    <button type="button" class="btn-delete-niveau flex items-center gap-x-2.5 w-full py-2 px-3 text-sm text-red-500 hover:bg-red-50 rounded-lg transition-colors" data-niveau-id="${niveau.id}">
